	...
	class WC_Prdctfltr {
		...
		public static function make_global( $set, $query = array() ) {
			...
			if ( $stop === false ) {
				...
				$orderby = self::get_catalog_ordering_args();
				$prdctfltr_global['default_order']['orderby'] = $orderby;

				if ( !isset( $set['orderby'] ) ) {
					$set['orderby'] = $orderby;
				}
...
/*
					if ( !is_object_in_taxonomy( 'product', $current->taxonomy ) ) {
						return;
					}
*/
					$current_terms = $wp_query->tax_query->queried_terms[$current->taxonomy]['terms'];

					if ( !empty( $wp->matched_query ) ) {
						if ( strpos( $wp->matched_query, '=' ) > 0 ) {
							parse_str( html_entity_decode( $wp->matched_query ), $query_array );
							$query = explode( '=', $wp->matched_query, 2 );

							if ( $current->taxonomy == $query[0] && count( $current_terms ) == 1 && $current->slug == $query_array[$query[0]] ) {
								$found_active[$current->taxonomy] = $current->slug;
								$prdctfltr_global['found_active'] = $found_active;
							}
							else if ( is_tax( $query[0] ) ) {
								$prdctfltr_global['found_active'][$query[0]] = $query_array[$query[0]];
							}
						}
					}

					if ( !isset( $found_active ) && isset( $_REQUEST[$current->taxonomy] ) ) {
						if ( strpos( $_REQUEST[$current->taxonomy], ',' ) || strpos( $_REQUEST[$current->taxonomy], '+' ) ) {
							...
							if ( $rewrite !== false ) {
								$redirect = preg_replace( '/\?.*/', '', get_bloginfo( 'url' ) ) . '/' . str_replace( '%' . $current->taxonomy . '%', $_REQUEST[$current->taxonomy], $rewrite );
							}
						}
						else {
...
<?php
		...
		public static function get_taxnomy_terms( $terms, $customization, $curr_include, $curr_fo, $curr_cat_selected, $output_terms ) {
			...
			foreach ( $terms as $term ) {
				...
				printf('<label class="%6$s%4$s%7$s%8$s"><input type="checkbox" value="%1$s"%3$s /><span>%2$s</span>%5$s</label>', $term->slug, $curr_insert, ( in_array( $term->slug, $curr_cat_selected ) ? ' checked' : '' ), ( in_array( $term->slug, $curr_cat_selected ) ? ' prdctfltr_active' : '' ), ( !empty( $term->children ) ? '<i class="prdctfltr-plus"></i>' : '' ), $pf_adoptive_class, ( !empty( $term->children ) && in_array( $term->slug, $curr_cat_selected ) ? ' prdctfltr_clicked' : '' ), ' prdctfltr_ft_' . sanitize_title( $term->slug ) );
				...
				if ( isset( $curr_fo['settings']['hierarchy'] ) && $curr_fo['settings']['hierarchy'] == 'yes' && !empty( $term->children ) ) {
					...
					self::get_taxnomy_terms( $term->children, $customization, $curr_include, $curr_fo, $curr_cat_selected, $output_terms );
